...
export default class PostEditMixin extends mixins(PostMixin, TermMixin) {
  ...
  onUpdatePostStatus(status: PostStatus) {
    return this.updatePostStatus(this.post.id, status)
      .then((result) => {
        if (!result) {
          this.$message.error(
            this.$tv(
  ...
  beforeThumbnailUpload(file: File) {
    ...
    if (!isJPGOrPNG) {
      this.$notification.error({
        message: this.$tv('core.page-post.tips.upload_image_type_limited', 'You can only upload JPG or PNG file!', {
          types: ['JPG', 'PNG'],
        }) as string,
        description: '',
      });
    ...
    if (!isLt2M) {
      this.$notification.error({
        message: this.$tv('core.page-post.tips.upload_image_size_limited', 'Image must smaller than 2MB!', {
          size: '2MB',
        }) as string,
        description: '',
      });
