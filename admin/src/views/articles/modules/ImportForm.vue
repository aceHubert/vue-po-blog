    ...
    <div class="table-page-search-wrapper">
      <a-modal v-model="drawerVisible" title="博客搬家">
        ...
        <a-spin tip="正在导入数据，请等待片刻" :spinning="spinning">
          <a-tabs defaultActiveKey="1" tabPosition="top" :style="{ height: '450px' }" @change="callback">
              ...
              <a-form :form="form" @submit="handleSubmit" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                <a-row class="form-row" :gutter="16">
                  <a-col :lg="24" :md="12" :sm="24">
                    ...
                    >
                      <a-input
                        v-decorator="['username', { rules: [{ required: true, message: '请输入mysql用户名!' }] }]"
                ...
                >
                  <a-input
                    v-decorator="['password', { rules: [{ required: true, message: '请输入mysql密码!' }] }]"
                    type="password"
                    placeholder="mysql密码"
                    :allowClear="allowClear"
                  >
                ...
                <a-form-item label="数据库">
                  <a-input
                    v-decorator="['database', { rules: [{ required: true, message: '请输入数据库名称!' }] }]"
                ...
                <a-form-item label="端口">
                  <a-input-number
                    ...
                    :min="1"
                    :max="65535"
                    v-decorator="['port', { rules: [{ required: true, message: '请输入数据库ip地址!' }] }]"
                    :step="1"
                    :precision="0"
                  />
                </a-form-item>
                <a-form-item label="ip">
                  <a-input
                    v-decorator="['ip', { rules: [{ required: true, message: '请输入数据库ip地址!' }] }]"
                    placeholder="ip"
                    :allowClear="allowClear"
                  >
                    <a-icon slot="prefix" type="lock" style="color: rgba(0, 0, 0, 0.25)" />
                ...
                <a-form-item label="博客">
                  <a-select
                    v-decorator="['blogType', { rules: [{ required: true, message: '请选择博客!' }] }]"
                    placeholder="请选择博客"
                  >
