    ...
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="8" :style="{ marginBottom: '24px' }">
        <chart-card
          :loading="loading"
          title="文章数量"
          :total="quantityObject.articleTotal"
          v-if="quantityObject !== null"
        >
          ...
          <template slot="footer"
            >今日发布<span> {{ quantityObject.todayPublishTotal }}</span></template
          >
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="8" :style="{ marginBottom: '24px' }">
        <chart-card
          :loading="loading"
          title="同步文章效果"
          :total="countPerson.viewsTotal | NumberFormat"
          v-if="countPerson !== null"
        >
          <a-tooltip title="指标说明" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
      ...
      <a-col :sm="24" :md="12" :xl="8" :style="{ marginBottom: '24px' }">
        <chart-card
          :loading="loading"
          title="系统信息"
          :total="memoryInfo.usedRatio + '%'"
          v-if="systemInfo !== null && memoryInfo !== null"
        >
          <a-tooltip title="指标说明" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
...
<script>
import { ChartCard, Ellipsis } from '@/components';
...
export default {
  ...
  methods: {
    ...
    dateChange(date, dateString) {
      ...
      this.$refs.viewChart.getPostsStatistics(
        'others',
